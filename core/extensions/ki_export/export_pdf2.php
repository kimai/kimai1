...
class MYPDF extends TCPDF { 
  ...
  function printExpenseRow($columns,$w,$row) {
    ...
    $event_string   = ($columns['action'] && !empty($row['evt_name'])) ? $kga['lang']['xp_ext']['expense'].': <i>'.$row['evt_name'].'</i>' : '';
    $user_string    = ($columns['user'] && !empty($row['username']))   ? $kga['lang']['xp_ext']['by'].': <i>'.$row['username'].'</i>'      : '';
    $comment_string = ($columns['comment'] && !empty($row['comment'])) ? $kga['lang']['comment'].': <i>'.nl2br($row['comment']).'</i>'     : '';
