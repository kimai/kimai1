...
class PostgreSQL 
{
	...
	private $pg_link     = 0;        // mysql link resource
	...
	public function GetColumnComments($table) {
		$this->ResetError();
		$records = pg_query(
                        "select pa.attname \"FIELD\", pd.description \"COMMENT\"
                        from pg_description pd, pg_class pc, pg_attribute pa, pg_namespace pn

                        where pc.relname = '".$table."'
                        and pn.nspname = '".$this->db_schema."'
		...
		if (! $records) {
			...
			if ($this->Error()) {
				...
				$index = 0;
				// Fetchs the array to be returned (column 1 is field comment):
				while ($array_data = pg_fetch_array($records)) {
					$columns[$index] = $array_data[1];
					$columns[$columnNames[$index++]] = $array_data[1];
				}
				return $columns;
			}
		}
	...
	public function GetColumnCount($table = "") {
		$this->ResetError();
		if (empty($table)) {
			$result = pg_num_fields($this->last_result);
			if (! $result) $this->SetError();
		} else {
			$records = pg_query("SELECT * FROM " . $table . " LIMIT 1");
			if (! $records) {
				$this->SetError();
				$result = false;
			} else {
				$result = pg_num_fields($records);
				$success = @pg_free_result($records);
				if (! $success) {
					$this->SetError();
					$result = false;
				}
		...
		if (empty($table)) {
			if ($this->RowCount() > 0) {
				if (is_numeric($column)) {
					return pg_field_type($this->last_result, $column);
				} else {
					return pg_field_type($this->last_result, $this->GetColumnID($column));
				}
			} else {
				return false;
			}
			...
			$result = pg_query("SELECT " . $column . " FROM " . $table . " LIMIT 1");
	...
	public function GetColumnLength($column, $table = "") {
		...
		if (empty($table)) {
			...
			if (! $columnID) {
				...
				$result = pg_field_size($this->last_result, $columnID);
				if (! $result) {
					$this->SetError();
					return false;
				} else {
			...
			$records = pg_query("SELECT " . $column . " FROM " . $table . " LIMIT 1");
			if (! $records) {
				$this->SetError();
				return false;
			}
			$result = pg_field_size($records, 0);
			if (! $result) {
				$this->SetError();
				return false;
			} else {
   ...
   /**
	...
	public function GetColumnName($columnID, $table = "") {
		...
		if (empty($table)) {
			if ($this->RowCount() > 0) {
				$result = pg_field_name($this->last_result, $columnID);
				if (! $result) $this->SetError();
			} else {
				$result = false;
			}
			...
			if (! $records) {
				...
				if (pg_num_fields($records) > 0) {
					$result = pg_field_name($records, $columnID);
					if (! $result) $this->SetError();
				} else {
					$result = false;
				}
	...
	public function GetColumnNames($table = "") {
		$this->ResetError();
		if (empty($table)) {
			$columnCount = pg_num_fields($this->last_result);
			if (! $columnCount) {
				$this->SetError();
				$columns = false;
			} else {
			...
			$result = pg_query(
                                "select pa.attname \"FIELD\"
                                from pg_class pc, pg_attribute pa, pg_namespace pn
			...
			if (! $result) {
				...
				while ($array_data = pg_fetch_array($result)) {
	...
	public function GetHTML($showCount = true, $styleTable = null, $styleHeader = null, $styleData = null) {
		...
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				...
				$this->MoveFirst();
				$header = false;
				while ($member = pg_fetch_object($this->last_result)) {
					if (!$header) {
						$html .= "\t<tr>\n";
	...
	*/
	public function GetJSON() {
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				for ($i = 0, $il = pg_num_fields($this->last_result); $i < $il; $i++) {
					$types[$i] = pg_field_type($this->last_result, $i);
				}
				$json = '[';
				$this->MoveFirst();
				while ($member = pg_fetch_object($this->last_result)) {
	...
	public function GetTables() {
		...
		$records = pg_query(
                        ...
                        where pn.nspname = '".$this->db_schema."'
                        and pc.relnamespace = pn.oid
                        and pc.relkind = 'r' ;");
		if (! $records) {
			$this->SetError();
	...
	public function GetXML() {
		...
		if (is_resource($this->last_result)) {
			...
			$rowCount = 0;
			while ($row = pg_fetch_assoc($this->last_result)) {
	 ...
	 */
	public function QueryArray($sql, $resultType = PGSQL_BOTH) {
		$this->Query($sql);
		if (! $this->Error()) {
	 ...
	 */
	public function QuerySingleRowArray($sql, $resultType = PGSQL_BOTH) {
		$this->Query($sql);
		if ($this->RowCount() > 0) {
	...
	public function QuerySingleValue($sql) {
		...
		if ($this->RowCount() > 0 && $this->GetColumnCount() > 0) {
			$row = $this->RowArray(null, PGSQL_NUM);
			return $row[0];
		} else {
			return false;
		}
	 ...
	 */
	public function RecordsArray($resultType = PGSQL_BOTH) {
		$this->ResetError();
		if ($this->last_result) {
			if (! @pg_result_seek($this->last_result, 0)) {
				$this->SetError();
				return false;
			} else {
	 ...
	 */
	public function RowArray($optional_row_number = null, $resultType = PGSQL_BOTH) {
		$this->ResetError();
		if (! $this->last_result) {
			$this->SetError("No query results exist", -1);
			return false;
	...
	public function RowCount() {
		...
		if (! $this->IsConnected()) {
			...
			$result = @pg_num_rows($this->last_result);
			if (! $result) {
				$this->SetError();
				return false;
			} else {
	...
	public function Seek($row_number) {
		...
		if (! $row_count) {
			...
			$result = pg_result_seek($this->last_result, $row_number);
			if (! $result) {
				$this->SetError();
				return false;
			} else {
				$record = pg_fetch_row($this->last_result);
				if (! $record) {
					$this->SetError();
					return false;
				} else {
					// Go back to the record after grabbing it
					pg_result_seek($this->last_result, $row_number);
					return $record;
				}
			}
		}
	...
	public function SelectDatabase($database, $charset = "") {
		...
		$this->ResetError();
		if (! (pg_query("set database ".$database." ;"))) {
			$this->SetError();
			$return_value = false;
		} else {
			if ((strlen($charset) > 0)) {
				if (! (pg_set_client_encoding($this->pg_link,$charset))) {
					$this->SetError();
					$return_value = false;
				}
			}
	...
	public function UpdateRows($tableName, $valuesArray, $whereArray = null) {
        ...
        /**
         * Drop in replace for mysql last inserted id
         * but needs the table name
         * @param type $table
         */
        public function PGLastId($table){
                ...
                if (! $records) {
                        ...
                        $array_data = pg_fetch_array($records);
                        $result = $array_data[0];
                }
