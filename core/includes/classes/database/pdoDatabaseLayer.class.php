...
class PDODatabaseLayer extends DatabaseLayer {
  ...
  public function connect($host,$database,$username,$password,$utf8,$serverType) {
    ...
    try {
      ...
      Logger::log('PDO CONNECTION FAILED: ' . $pdo_ex->getMessage());
    }
  }
  ...
  */
  public function zef_get_data($zef_id) {
      ...
      if ($zef_id) {
          ...
          // Logger::logfile("SELECT * FROM " . $this->kga['server_prefix'] . "zef ORDER BY zef_ID DESC LIMIT 1");
      }
  ...
  */
  public function zef_create_record($usr_ID,$data) {
    ...
    Logger::logfile(serialize($pdo_query->errorInfo()));
      if ($result === false)
          return false;
      else
        return $this->conn->lastInsertId();
  ...
  * @author th
  */
  public function get_entry_zef($id) { 
      $p = $this->kga['server_prefix'];

      $pdo_query = $this->conn->prepare("SELECT * FROM ${p}zef 
  ...
  */
  public function get_current_timer() {
      ...
      $pdo_query->execute(array($this->kga['usr']['usr_ID']));
      $row = $pdo_query->fetch(PDO::FETCH_ASSOC);
  ...
  */
  public function get_zef_time_day($inPoint,$user) {
      ...
      $pdo_query = $this->conn->prepare("SELECT sum(zef_time) as zeit FROM ${p}zef WHERE zef_in > ? AND zef_out < ? AND zef_usrID = ?;");
  ...
  */
  public function get_zef_time_all($user) {
      ...
      $pdo_query = $this->conn->prepare("SELECT sum(zef_time) as zeit FROM ${p}zef WHERE zef_usrID = ?");
