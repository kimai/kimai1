...
/**
 * THE DAY-EXPLODER (TM) - Makes Your Day!!
 * tries to solve the cross-midnight-problem without coffie
 *
 * hier wird gefragt ob
 * die zeitspanne von in bis out (unix seconds) nur innerhalb
 * der grenzen eines kalendertages liegt.
 * ist das nicht der fall wird der zeitraum auf mehrere tage verteilt
 * idealer output wäre ein multidimensionales array der tage
 * mit entsprechenden in und out values
 *
 * sollte sowohl auf die direkt aufgenommenen records als auch
 * auf die editierten anwendbar sein
 *
 * <pre>
 * returns:
 * [0]['in'] -> in unix seconds of day 1
 * [0]['out']
 * [0]['diff'] -> space between in and out
 * [1]['in'] -> in unix seconds of day 2
 * [1]['out']
 * ...
 * </pre>
 *
 ...
 * @author th
 */
function explode_record($in,$out) {
// solves the cross-midnight-bug
    $in_date  = date("d",$in);
    $out_date = date("d",$out);
    
    $span_of_days = $out_date - $in_date;
    
    // if the recording does not cross midnight
    // then return the given values without any change - via array .
    if ($span_of_days == 0) {
        $arr[0]['in']   = $in;
        $arr[0]['out']  = $out;
        $arr[0]['diff'] = $out - $in;

    // ... but if the start date differs from the end date -
    // return multiple dates via array.
    } else {
    
        // bisher geht nur ein tag ....
        // man kann also noch nicht 2 tage ununterbrochen durcharbeiten ;)
        // TODO: umschreiben (dynamisch)
        $arr[0]['in']  = $in;
        $arr[0]['out'] = mktime(23,59,59,date("n",$in),date("d",$in),date("Y",$in));
        $arr[0]['diff'] = $arr[0]['out'] - $arr[0]['in'];
        
        $arr[1]['in']  = mktime(0,0,0,date("n",$in),date("d",$in)+1,date("Y",$in));
        $arr[1]['out'] = $out;
        $arr[1]['diff'] = $arr[1]['out'] - $arr[1]['in'];
